...
class Tokenizer:
    ...
    def next(self):
        ...
        while True:
            if len(self._line) == 0:
                ...
                i = self._line.find("//")
                if not self._comment and i != -1: self._line = self._line[:i]
    ...
    def _parseInt(self):
        ...
        if len(self._line) > 0 and self._line[0].isalpha():
            raise JackError('Nije int')
        
        if ret >= 32768:
            raise JackError("Broj je prevelik")
